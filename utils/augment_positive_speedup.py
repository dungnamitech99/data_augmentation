...
if __name__ == "__main__":
    ...
    for audio_file in positive_paths:
        ...
        if audio_name not in audio_names:
            change_speed(audio_file, f"ftel5_positive_speed_up/{audio_name}", ratio)
            audio_names.append(audio_name)

    speed_up_audios = glob("ftel5_positive_speed_up/*.wav")
    for speed_up_audio in speed_up_audios:
        x, sr = audioread(speed_up_audio)
        audio_name = speed_up_audio.split("/")[-1]
        padded_samples_front = round((word_end_time - word_end_time / ratio) * sr)
        x = np.append(np.zeros(padded_samples_front), x)
        padded_samples_back = round(3 * sr - len(x))
        x = np.append(x, np.zeros(padded_samples_back))
        audiowrite(x, sr, f"ftel5_positive_speed_up/{audio_name}")
